...
parser.add_argument("--table_path", type=str, required=True, help="Path to TSV file containing 'id' and 'path' columns, e.g., 'eqtl_paths.tsv'.")
parser.add_argument("--output_dir", type=str, required=True, help="Directory to store signal files, e.g., 'eqtl_signals'.")
parser.add_argument("--summary", type=str, required=True, help="File to write the summary, e.g., 'eqtl_summary.tsv'.")

args = parser.parse_args()

metadata = pd.read_csv(args.table_path, sep='\t')
...
for row in tqdm(metadata.itertuples(index=False), desc="Processing files"):
    ...
    process_gwas_file(file_path, dataset_id, args.output_dir, args.summary)